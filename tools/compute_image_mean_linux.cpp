...
int compute_imapge_mean_linux(int argc, char** argv) {
  ...
  if (db_backend == "leveldb") {  // leveldb
    ...
    CAFFE_CHECK_EQ(mdb_env_create(&mdb_env), MDB_SUCCESS); // << "mdb_env_create failed";
    CAFFE_CHECK_EQ(mdb_env_set_mapsize(mdb_env, 1099511627776), MDB_SUCCESS);  // 1TB
    CAFFE_CHECK_EQ(mdb_env_open(mdb_env, argv[1], MDB_RDONLY, 0664),
        MDB_SUCCESS); // << "mdb_env_open failed";
    CAFFE_CHECK_EQ(mdb_txn_begin(mdb_env, NULL, MDB_RDONLY, &mdb_txn), MDB_SUCCESS); //
        //<< "mdb_txn_begin failed";
    CAFFE_CHECK_EQ(mdb_open(mdb_txn, NULL, 0, &mdb_dbi), MDB_SUCCESS); //
        //<< "mdb_open failed";
    CAFFE_CHECK_EQ(mdb_cursor_open(mdb_txn, mdb_dbi, &mdb_cursor), MDB_SUCCESS); //
        //<< "mdb_cursor_open failed";
    CAFFE_CHECK_EQ(mdb_cursor_get(mdb_cursor, &mdb_key, &mdb_value, MDB_FIRST),
        MDB_SUCCESS);
  } else {
