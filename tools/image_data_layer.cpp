...
namespace caffe {
	...
	void* ImageDataLayerPrefetch(void* layer_pointer) {
		...
		if (mirror && crop_size == 0) {
			std::cout << std::endl << "Current implementation requires mirror and crop_size to be "
		...
		for (int item_id = 0; item_id < batch_size; ++item_id) {
			...
			if (layer->lines_id_ >= lines_size) {
				...
				std::cout << std::endl << "Restarting data prefetching from start.";
				layer->lines_id_ = 0;
	...
	void ImageDataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
		vector<Blob<Dtype>*>* top) {
			...
			std::cout << std::endl << "Opening file " << source;
			...
			if (this->layer_param_.image_data_param().shuffle()) {
				...
				std::cout << std::endl << "Shuffling data";
				const unsigned int prefetch_rng_seed = caffe_rng_rand();
				prefetch_rng_.reset(new Caffe::RNG(prefetch_rng_seed));
				ShuffleImages();
			}
			std::cout << std::endl << "A total of " << lines_.size() << " images.";
			...
			if (this->layer_param_.image_data_param().rand_skip()) {
				...
				std::cout << std::endl << "Skipping first " << skip << " data points.";
			...
			std::cout << std::endl << "output data size: " << (*top)[0]->num() << ","
			...
			if (this->layer_param_.image_data_param().has_mean_file()) {
				...
				std::cout << std::endl << "Loading mean file from" << mean_file;
			...
			data_mean_.cpu_data();
			std::cout << std::endl << "Initializing prefetch";
			CreatePrefetchThread();
			std::cout << std::endl << "Prefetch initialized.";
	}
