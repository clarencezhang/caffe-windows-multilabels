...
namespace caffe {
	...
	Dtype ConvolutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		vector<Blob<Dtype>*>* top) {
			...
			//K_ = channels_ * kernel_size_ * kernel_size_ / group_;
			//N_ = height_out * width_out;
			int weight_offset = M_ * K_; // (num_output_ / group_) * 
			                             // (channels_ * kernel_size_ * kernel_size_ / group_)
			                             
			int col_offset = K_ * N_;    // (channels_ * kernel_size_ * kernel_size_ / group_) * (height_out * width_out)

			int top_offset = M_ * N_;    // (num_output_ / group_) * (height_out * width_out)
			...
			for (int n = 0; n < num_; ++n) {
				...
				// Weight Blob£º50¡Á20¡Á5 ¡Á5 (num_output_ * channels_ * kernel_size_ * kernel_size_)
