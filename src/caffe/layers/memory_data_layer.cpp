...
namespace caffe {
	...
	void MemoryDataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
		vector<Blob<Dtype>*>* top) {
			CAFFE_CHECK_EQ(bottom.size(), 0); // << "Memory Data Layer takes no blobs as input.";
			CAFFE_CHECK_EQ(top->size(), 2); // << "Memory Data Layer takes two blobs as output.";
			...
			CAFFE_CHECK_GT(batch_size_ * datum_size_, 0); // << "batch_size, channels, height,"
	...
	void MemoryDataLayer<Dtype>::Reset(Dtype* data, Dtype* labels, int n) {
		CAFFE_CHECK(data);
		CAFFE_CHECK(labels);
		CAFFE_CHECK_EQ(n % batch_size_, 0); // << "n must be a multiple of batch size";
		data_ = data;
		labels_ = labels;
		n_ = n;
		pos_ = 0;
	...
	Dtype MemoryDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		vector<Blob<Dtype>*>* top) {
			CAFFE_CHECK(data_); // << "MemoryDataLayer needs to be initalized by calling Reset";
