...
namespace caffe {
	...
	void im2col_cpu(const Dtype* data_im, const int channels,
		...
		const int stride, Dtype* data_col) {
			...
			// === The same as height_out & width_out ===
			...
			// col_buffer_.Reshape(1, channels_ * kernel_size_ * kernel_size_, height_out, width_out);
			// row_index(c), row_size = channels_col = channels * ksize * ksize
			// col_index(h,w) = h * width_col + w, col_size = height_col * width_col
			// (c * height_col * width_col) + (h * width_col + w)
